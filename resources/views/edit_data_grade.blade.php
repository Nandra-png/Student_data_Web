<x-admin-layout>
    ...
    <div class="bg-white rounded-lg shadow-lg">
        <div class="p-6 border-b border-gray-200 flex items-center gap-3">
            ...
            <h2 class="text-2xl font-bold text-gray-800">Edit Grade: {{ $grade_student->name }}</h2>
        </div>

        <div class="p-6">
            <form action="{{ route('grade.update', $grade_student->id) }}" method="POST" class="max-w-3xl mx-auto space-y-6">
                @csrf
                @method('PUT')
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    <div class="sm:col-span-2">
                        ...
                        <input type="text"
                               name="name"
                               id="name"
                               value="{{ old('name', $grade_student->name) }}"
                               class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                               placeholder="Enter grade name (e.g. 10 PPLG 1)"
                               required>
                        @error('name')
                    ...
                    <div class="sm:col-span-2">
                        <label for="departmen_id" class="block mb-2 text-sm font-medium text-gray-900">Department</label>
                        <select id="departmen_id"
                                name="departmen_id"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                            <option value="">Select department</option>
                            @foreach($departmens as $department)
                                <option value="{{ $department->id }}" {{ (old('departmen_id', $grade_student->departmen_id) == $department->id) ? 'selected' : '' }}>
                                    {{ $department->name }}
                                </option>
                            @endforeach
                        </select>
